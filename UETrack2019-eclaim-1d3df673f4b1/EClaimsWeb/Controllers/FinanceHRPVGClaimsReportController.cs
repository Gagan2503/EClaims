...
{
    public class FinanceHRPVGClaimsReportController : Controller
    {
        ...
        public FinanceHRPVGClaimsReportController(ILoggerManager logger, IRepositoryWrapper repository, IMapper mapper, RepositoryContext context, IConfiguration configuration, IConverter converter)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> Index(int userID, string moduleName, int facilityID, int statusID, string fromDate, string toDate)
        {
            try
            {
                //if(ddlReportType == "1")
                ...
                oclsModule.Add(new clsModule() { ModuleName = "HR PV-GIRO Claim", ModuleId = "HRPV-GIROClaim" });
                ...
                var mstMileageClaimsWithDetails = await _repository.MstMileageClaim.GetAllUserHRPVGClaimsReportAsync(userID, "", moduleName, facilityID, statusID, fromDate, toDate);
                foreach (var mc in mstMileageClaimsWithDetails)
                {
                    ...
                    if (!string.IsNullOrEmpty(mc.CreatedDate))
                    {
                        mileageClaimVM.CreatedDate = DateTime.ParseExact(mc.CreatedDate, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture)
                                                           .ToString("dd/MM/yyyy", System.Globalization.CultureInfo.CreateSpecificCulture("en-US"));
                    }

                    if (!string.IsNullOrEmpty(mc.ApprovalDate))
                    {
                        mileageClaimVM.ApprovalDate = DateTime.ParseExact(mc.ApprovalDate, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture)
                                                             .ToString("dd/MM/yyyy", System.Globalization.CultureInfo.CreateSpecificCulture("en-US"));
                    }

                    if (!string.IsNullOrEmpty(mc.ExportAccPacDate))
                    {
                        mileageClaimVM.ExportAccPacDate = DateTime.ParseExact(mc.ExportAccPacDate, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture)
                                                             .ToString("dd/MM/yyyy", System.Globalization.CultureInfo.CreateSpecificCulture("en-US"));
                    }

                    if (!string.IsNullOrEmpty(mc.ExportBankDate))
                    {
                        mileageClaimVM.ExportBankDate = DateTime.ParseExact(mc.ExportBankDate, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture)
                                                             .ToString("dd/MM/yyyy", System.Globalization.CultureInfo.CreateSpecificCulture("en-US"));
                    }
        ...
        public async Task<JsonResult> ExporttoExcel(string data)
        {
            ...
            string filename = "FinanceHRPVGClaims-Report-" + DateTime.Now.ToString("ddMMyyyyss") + ".xlsx";
            ...
            var customClaimReports = await _repository.MstMileageClaim.GetAllUserHRPVGClaimsReportAsync(Int32.Parse(aPExportSearch.UserID), "", aPExportSearch.ModuleName, Int32.Parse(aPExportSearch.FacilityID), Int32.Parse(aPExportSearch.StatusID), aPExportSearch.FromDate, aPExportSearch.ToDate);
            ...
            dt.Rows.Add("", "", "", "", "", "", "", "", "Total", ClaimsGrandTotal, "");
        ...
        public async Task<IActionResult> Download(string fileName)
        {
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        ...
                        return File(blobStream, file.Properties.ContentType, "FinanceHRPVGClaimsReport.xlsx");
                    }
                    else
                    {
            ...
            //return File(fileByteArray, "application/vnd.ms-excel", "FinanceHRPVGClaimsReport.xlsx");
        }
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        await file.DownloadToStreamAsync(ms);
                        Stream blobStream = file.OpenReadAsync().Result;
                        return File(blobStream, file.Properties.ContentType, "FinanceHRPVGClaimsReport.pdf");
                    }
                    else
                    {
                        return Content("File does not exist");
        ...
        private byte[] BuildPdf(string HtmlContent, string Width, string Height, MarginSettings Margins, int? DPI = 180)
        {
            ...
            return _converter.Convert(new HtmlToPdfDocument()
            {
                ...
                GlobalSettings = new GlobalSettings
                {
                    ...
                    DocumentTitle = "User HR PV GIRO Claims Report"
                }
            });
        }
        ...
        {
            var aPExportSearch = JsonConvert.DeserializeObject<APExportSearch>(data);
            string filename = "FinanceHRPVGClaims-Report-" + DateTime.Now.ToString("ddMMyyyyss") + ".pdf";
            var path = "FileUploads/temp/";
            string pathToFilesold = System.IO.Path.Combine(path, filename);
            //FileInfo newFile = new FileInfo(pathToFilesold);
            //if (newFile.Exists)
            ...
            sb.Append(@"
                        <html>
                            ...
                            <body>
                                <div class='header'><h1>User HR PV-GIRO Claims Report</h1></div>
                                <table align='center'>
                                    <tr>
                                        <th>Date Created</th>
                                        <th>Voucher No</th>
            ...
            foreach (var mc in customClaimReports)
            {
                ...
                sb.AppendFormat(@"<tr>
                                    ...
                                    <td>{9}</td>
                                    <td>{10}</td>
                                  </tr>", Convert.ToDateTime(mc.CreatedDate).ToString("dd/MM/yyyy", System.Globalization.CultureInfo.CreateSpecificCulture("en-US")), mc.VoucherNo, mc.CNO, mc.Name, mc.PayeeName, mc.Description, mc.AccountCode,
                              mc.FacilityName, mc.ExpenseCategory, mc.GrandTotal, mc.ClaimStatusName);
            }
            sb.AppendFormat(@"<tr>
                                    <td>{0}</td>
                                    <td>{1}</td>
                                    ...
                                    <td>{9}</td>
                                    <td>{10}</td>
                                  </tr>", string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "Total ", ClaimsGrandTotal, string.Empty);
            sb.Append(@"
                                </table>
                            </body>
                        </html>");
