...
{
    public class FinanceUserClaimsReportController : Controller
    {
        ...
        public FinanceUserClaimsReportController(ILoggerManager logger, IRepositoryWrapper repository, IMapper mapper, RepositoryContext context, IConfiguration configuration, IConverter converter)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> Index(int userID, string moduleName, int facilityID, int statusID, string fromDate, string toDate)
        {
            try
            {
                ...
                var mstMileageClaimsWithDetails = await _repository.MstMileageClaim.GetAllUserClaimsReportAsync(userID, "", moduleName, facilityID, statusID, fromDate, toDate);
        ...
        public async Task<JsonResult> ExporttoExcel(string data)
        {
            ...
            var customClaimReports = await _repository.MstMileageClaim.GetAllUserClaimsReportAsync(Int32.Parse(aPExportSearch.UserID), "", aPExportSearch.ModuleName, Int32.Parse(aPExportSearch.FacilityID), Int32.Parse(aPExportSearch.StatusID), aPExportSearch.FromDate, aPExportSearch.ToDate);
        ...
        public async Task<IActionResult> Download(string fileName)
        {
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        ...
                        return File(blobStream, file.Properties.ContentType, "FinanceUserClaimsReport.xlsx");
                    }
                    else
                    {
            ...
            //return File(fileByteArray, "application/vnd.ms-excel", "FinanceUserClaimsReport.xlsx");
        }
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        await file.DownloadToStreamAsync(ms);
                        Stream blobStream = file.OpenReadAsync().Result;
                        return File(blobStream, file.Properties.ContentType, "FinanceUserClaimsReport.pdf");
                    }
                    else
                    {
                        return Content("File does not exist");
        ...
        {
            var aPExportSearch = JsonConvert.DeserializeObject<APExportSearch>(data);
            string filename = "FinanceUserClaims-Report-" + DateTime.Now.ToString("ddMMyyyyss") + ".pdf";
            var path = "FileUploads/temp/";
            string pathToFilesold = System.IO.Path.Combine(path, filename);
            //FileInfo newFile = new FileInfo(pathToFilesold);
            //if (newFile.Exists)
            ...
            //var path1 = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "FileUploads", "temp");
