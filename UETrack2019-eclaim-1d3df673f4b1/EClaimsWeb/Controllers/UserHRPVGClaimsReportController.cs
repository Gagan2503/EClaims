...
{
    public class UserHRPVGClaimsReportController : Controller
    {
        ...
        public UserHRPVGClaimsReportController(ILoggerManager logger, IRepositoryWrapper repository, IMapper mapper, RepositoryContext context, IConfiguration configuration, IConverter converter)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> Index(string moduleName, int facilityID, int statusID, string fromDate, string toDate)
        {
            try
            {
                ...
                var mstMileageClaimsWithDetails = await _repository.MstMileageClaim.GetAllUserHRPVGClaimsReportAsync(Convert.ToInt32(HttpContext.User.FindFirst("userid").Value), "", moduleName, facilityID, statusID, fromDate, toDate);
        ...
        public async Task<JsonResult> ExporttoExcel(string data)
        {
            ...
            string filename = "UserHRPVGClaims-Report-" + DateTime.Now.ToString("ddMMyyyyss") + ".xlsx";
            ...
            var customClaimReports = await _repository.MstMileageClaim.GetAllUserHRPVGClaimsReportAsync(Convert.ToInt32(HttpContext.User.FindFirst("userid").Value), "", aPExportSearch.ModuleName, Int32.Parse(aPExportSearch.FacilityID), Int32.Parse(aPExportSearch.StatusID), aPExportSearch.FromDate, aPExportSearch.ToDate);
        ...
        public async Task<IActionResult> Download(string fileName)
        {
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        ...
                        return File(blobStream, file.Properties.ContentType, "UserHRPVGClaimsReport.xlsx");
                    }
                    else
                    {
            ...
            //return File(fileByteArray, "application/vnd.ms-excel", "UserHRPVGClaimsReport.xlsx");
        }
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        await file.DownloadToStreamAsync(ms);
                        Stream blobStream = file.OpenReadAsync().Result;
                        return File(blobStream, file.Properties.ContentType, "UserHRPVGClaimsReport.pdf");
                    }
                    else
                    {
                        return Content("File does not exist");
        ...
        {
            var aPExportSearch = JsonConvert.DeserializeObject<APExportSearch>(data);
            string filename = "UserHRPVGClaims-Report-" + DateTime.Now.ToString("ddMMyyyyss") + ".pdf";
            var path = "FileUploads/temp/";
            string pathToFilesold = System.IO.Path.Combine(path, filename);
            //FileInfo newFile = new FileInfo(pathToFilesold);
            //if (newFile.Exists)
