...
{
    public class FinanceUserPVCClaimsReportController : Controller
    {
        ...
        public FinanceUserPVCClaimsReportController(ILoggerManager logger, IRepositoryWrapper repository, IMapper mapper, RepositoryContext context, IConfiguration configuration, IConverter converter)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> Index(int userID, string moduleName, int facilityID, int statusID, string fromDate, string toDate)
        {
            try
            {
                ...
                var mstMileageClaimsWithDetails = await _repository.MstMileageClaim.GetAllUserPVCClaimsReportAsync(userID, "", moduleName, facilityID, statusID, fromDate, toDate);
        ...
        public async Task<JsonResult> ExporttoExcel(string data)
        {
            ...
            string filename = "UserPVCClaims-Report-" + DateTime.Now.ToString("ddMMyyyyss") + ".xlsx";
            ...
            var customClaimReports = await _repository.MstMileageClaim.GetAllUserPVCClaimsReportAsync(Int32.Parse(aPExportSearch.UserID), "", aPExportSearch.ModuleName, Int32.Parse(aPExportSearch.FacilityID), Int32.Parse(aPExportSearch.StatusID), aPExportSearch.FromDate, aPExportSearch.ToDate);
        ...
        public async Task<IActionResult> Download(string fileName)
        {
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        ...
                        return File(blobStream, file.Properties.ContentType, "FinanceUserPVCClaimsReport.xlsx");
                    }
                    else
                    {
            ...
            //return File(fileByteArray, "application/vnd.ms-excel", "FinanceUserPVCClaimsReport.xlsx");
        }
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        await file.DownloadToStreamAsync(ms);
                        Stream blobStream = file.OpenReadAsync().Result;
                        return File(blobStream, file.Properties.ContentType, "FinanceUserPVCClaimsReport.pdf");
                    }
                    else
                    {
                        return Content("File does not exist");
        ...
        private byte[] BuildPdf(string HtmlContent, string Width, string Height, MarginSettings Margins, int? DPI = 180)
        {
            ...
            return _converter.Convert(new HtmlToPdfDocument()
            {
                ...
                GlobalSettings = new GlobalSettings
                {
                    ...
                    DocumentTitle = "User PV Cheque Claims Report"
                }
            });
        }
        ...
        {
            var aPExportSearch = JsonConvert.DeserializeObject<APExportSearch>(data);
            string filename = "FinanceUserPVCClaims-Report-" + DateTime.Now.ToString("ddMMyyyyss") + ".pdf";
            var path = "FileUploads/temp/";
            string pathToFilesold = System.IO.Path.Combine(path, filename);
            //FileInfo newFile = new FileInfo(pathToFilesold);
            //if (newFile.Exists)
            ...
            sb.Append(@"
                        <html>
                            ...
                            <body>
                                <div class='header'><h1>User PV-Cheque Claims Report</h1></div>
                                <table align='center'>
                                    <tr>
                                        <th>Date Created</th>
                                        <th>Voucher No</th>
            ...
            foreach (var mc in customClaimReports)
            {
                ...
                sb.AppendFormat(@"<tr>
                              ...
                              mc.FacilityName, mc.ExpenseCategory,mc.ChequeNo, mc.Description, mc.GrandTotal, mc.TotalAmount - mc.GrandTotal, mc.TotalAmount, mc.ClaimStatusName);
            }
            sb.AppendFormat(@"<tr>
                                    <td>{0}</td>
                                    <td>{1}</td>
                                    ...
                                    <td>{12}</td>
                                    <td>{13}</td>
                                  </tr>", string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "Total ", ClaimsGrandTotal, ClaimsTotalAmount - ClaimsGrandTotal, ClaimsTotalAmount, string.Empty);
            sb.Append(@"
                                </table>
                            </body>
                        </html>");
