...
{
    public class FinanceHRPVCClaimsReportController : Controller
    {
        ...
        public FinanceHRPVCClaimsReportController(ILoggerManager logger, IRepositoryWrapper repository, IMapper mapper, RepositoryContext context, IConfiguration configuration, IConverter converter)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> Index(int userID, string moduleName, int facilityID, int statusID, string fromDate, string toDate)
        {
            try
            {
                ...
                oclsModule.Add(new clsModule() { ModuleName = "HR PV-Cheque Claim", ModuleId = "HRPV-ChequeClaim" });
                ...
                var mstMileageClaimsWithDetails = await _repository.MstMileageClaim.GetAllUserHRPVCClaimsReportAsync(userID, "", moduleName, facilityID, statusID, fromDate, toDate);
                foreach (var mc in mstMileageClaimsWithDetails)
                {
                    ...
                    mileageClaimVM.ChequeNo = mc.ChequeNo;
        ...
        public async Task<JsonResult> ExporttoExcel(string data)
        {
            ...
            string filename = "FinanceHRPVCClaims-Report-" + DateTime.Now.ToString("ddMMyyyyss") + ".xlsx";
            ...
            var customClaimReports = await _repository.MstMileageClaim.GetAllUserHRPVCClaimsReportAsync(Int32.Parse(aPExportSearch.UserID), "", aPExportSearch.ModuleName, Int32.Parse(aPExportSearch.FacilityID), Int32.Parse(aPExportSearch.StatusID), aPExportSearch.FromDate, aPExportSearch.ToDate);
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        ...
                        return File(blobStream, file.Properties.ContentType, "FinanceHRPVCClaimsReport.xlsx");
                    }
                    else
                    {
            ...
            if (CloudStorageAccount.TryParse(_configuration.GetSection("ConnectionStrings")["BlobConnectionString"], out CloudStorageAccount storageAccount))
            {
                ...
                if (await container.ExistsAsync())
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        await file.DownloadToStreamAsync(ms);
                        Stream blobStream = file.OpenReadAsync().Result;
                        return File(blobStream, file.Properties.ContentType, "FinanceHRPVCClaimsReport.pdf");
                    }
                    else
                    {
                        return Content("File does not exist");
        ...
        private byte[] BuildPdf(string HtmlContent, string Width, string Height, MarginSettings Margins, int? DPI = 180)
        {
                ...
                Objects =
            {
                ...
                GlobalSettings = new GlobalSettings
                {
                    ...
                    DocumentTitle = "User HR PV Cheque Claims Report"
                }
            });
        }
        ...
        {
            var aPExportSearch = JsonConvert.DeserializeObject<APExportSearch>(data);
            string filename = "FinanceHRPVCClaims-Report-" + DateTime.Now.ToString("ddMMyyyyss") + ".pdf";
            var path = "FileUploads/temp/";
            string pathToFilesold = System.IO.Path.Combine(path, filename);
            //FileInfo newFile = new FileInfo(pathToFilesold);
            //if (newFile.Exists)
            ...
            sb.Append(@"
                        <html>
                            ...
                            <body>
                                <div class='header'><h1>User HR PV-Cheque Claims Report</h1></div>
                                <table align='center'>
                                    <tr>
                                        <th>Date Created</th>
                                        <th>Voucher No</th>
            ...
            foreach (var mc in customClaimReports)
            {
                ...
                sb.AppendFormat(@"<tr>
                              ...
                              mc.FacilityName, mc.ExpenseCategory, mc.ChequeNo, mc.GrandTotal, mc.ClaimStatusName);
            }
            sb.AppendFormat(@"<tr>
                                    <td>{0}</td>
                                    <td>{1}</td>
                                    ...
                                    <td>{10}</td>
                                    <td>{11}</td>
                                  </tr>", string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "Total ", ClaimsGrandTotal, string.Empty);
            sb.Append(@"
                                </table>
                            </body>
                        </html>");
